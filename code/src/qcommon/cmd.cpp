// cmd.cpp -- Black Ops II script command processing module
...
static unsigned char sv_cmd_text_buf[MAX_CMD_BUFFER];
static CmdText sv_cmd_text;
static CmdText cmd_textArray[1];
static unsigned char cmd_text_buf[1][MAX_CMD_BUFFER];
...
void Cmd_ExecAddText_f()
{
	...
	if (Cmd_Argc() != 2)
	{
		Com_Printf(
			...
			"exec_addtext <filename> : execute a script file, appending the contents to the command buffer\n");
		return;
	}
